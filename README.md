...
Полнофункциональный бот для управления ПК через Telegram с безопасной системой авторизации, системным треем и автозагрузкой.
...
- **Сетевая информация** (`/network`) - IP адреса и сетевые интерфейсы
...
### 📁 Файловая система
- **Открыть файл** (`/open <путь>`) - Запуск любого файла
- **Список файлов** (`/files <путь>`) - Просмотр содержимого папок

### � Новые возможности v2.0+

#### 📱 Системный трей
- **Иконка в трее** - Управление ботом из системного трея
- **Цветовая индикация** - Синяя (запущен) / Красная (остановлен)
- **Контекстное меню** - Полное управление одним кликом

#### 🔄 Автозагрузка
- **Автоматический запуск** - Запуск при старте Windows
- **Скрытый режим** - Работа в фоне без окна
- **Легкая настройка** - Включение/отключение через меню
